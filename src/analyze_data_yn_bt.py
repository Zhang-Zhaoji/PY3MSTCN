...
for idx, video_name in enumerate(video_names):
    ...
    if name in self_saliency_dict_names:
        ...
        anno_dicts.append(dict_version[name])
...
def build_df(anno, cause_iou, effect_iou, model_name):
    ...
    def get_label(element):
        label = 'self' if element['self_involved'] else 'other'
        label += '-involved\n'
        label += 'non-visual cue' if element['topdown'] else 'explicit visual cue'
        return label
    cause_rows, effect_rows = [], []
    for idx, element in enumerate(anno):
        # sample[1] -> cause, sample[2] -> effect
        # cause_label  = sample[1][0]
        # effect_label = sample[2][0]
        cause_label  =  effect_label = get_label(element)
        cause_rows.append({
            'label': cause_label,
            'iou':   cause_iou[idx],
            'model': model_name
...
def plot_violin(df, task='cause'):
    ...
    plt.title(f'{task.capitalize()} IoU distribution by category\n'
              '(labels sorted by SM - RGB mean IoU, descending)')
    plt.xlabel('IoU')
    plt.ylabel('')
