...
class Config(object):
    def __init__(self, cfg_file):
        ...
        self.epoch = self.cfg["epochs"] if "epochs" in self.cfg else 200
        self.lr = self.cfg["lr"] if "lr" in self.cfg else 0.0001
        ...
        self.num_workers = self.cfg["num_workers"] if "num_workers" in self.cfg else 6
        self.weight_decay = self.cfg["weight_decay"] if "weight_decay" in self.cfg else 0.01
        self.optimizer = self.cfg["optimizer"] if "optimizer" in self.cfg else "AdamW"
        ...
        self.num_layers = self.cfg["num_layers"] if "num_layers" in self.cfg else 2
        self.num_f_maps = self.cfg["num_f_maps"] if "num_f_maps" in self.cfg else 128
        self.dim = self.cfg["dim"] if "dim" in self.cfg else 1024
        self.num_classes = self.cfg["num_classes"] if "num_classes" in self.cfg else 3
